...
const createAula = async (req, res) => {
  const { id_turma, id_func_responsavel, start, end, desc, color, tipo } = req.body;
...
const updateAula = async (req, res) => {
  ...
  try {
    ...
    await aulaservices.updateAula(id, id_turma, id_func_responsavel, start, end, desc, color, tipo);
...
const cancelarAula = async (req, res) => {
  ...
  try {
    ...
    const aula = await aulaservices.getAulaById(id);
    if (!aula) {
      return res.status(404).json({ message: 'Não foi possível cancelar essa aula' });
    }
...
const getHistoricoByTurmaId = async (req, res) => {
  ...
  try {
    ...
    if (!historico || historico.length === 0) {
      return res.status(404).json({ message: "Histórico não encontrado para esta turma." });
    }
