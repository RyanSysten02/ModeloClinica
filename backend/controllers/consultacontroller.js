...
const createConsulta = async (req, res) => {
    const { id_aluno,id_func_responsavel, start, end, desc, color, tipo } = req.body;
    ...
    try {
        ...
        const consultaId = await consultasservices.createConsulta(id_aluno,id_func_responsavel, start, end, desc, color, tipo, id_usuario_inclusao);
...
const updateConsulta = async (req, res) => {
    ...
    try {
        ...
        await consultasservices.updateConsulta(id, id_aluno,id_func_responsavel, start, end, desc, color, tipo);
...
const getHistoricoByAlunoId = async (req, res) => {
    const { id } = req.params; // ID do aluno
    try {
      const historico = await consultasservices.getHistoricoConsultasByAlunoId(id);
      if (!historico || historico.length === 0) {
        return res.status(404).json({ message: "Histórico não encontrado para este aluno." });
      }
