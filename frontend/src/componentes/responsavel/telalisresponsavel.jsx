...
const TelaListaResponsavels = ({ onSelectResponsavel }) => {
  ...
  const fetchResponsavels = async () => {
    try {
      ...
      const response = await fetch("http://localhost:5001/api/responsavel/allresponsavel", {
        headers: {
  ...
  const handleDetalhes = async (id) => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/responsavel/responsavel/${id}`, {
        headers: {
          "Authorization": `Bearer ${token}`,
        },
      });
...
const handleHistorico = async (id) => {
  ...
  try {
    ...
    if (!response.ok) {
      toast.warning(data.message || "Erro ao carregar histórico do responsavel.");
      return;
    }
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/responsavel/responsavel/${formData.id}`, {
        method: 'PUT',
        headers: {
        ...
        <tbody>
          {responsavels.map((responsavel) => (
            <tr key={responsavel.id}>
              ...
              <td>
                <Button variant="primary" onClick={() => handleDetalhes(responsavel.id)}>
                  Detalhes
                </Button>
                <Button variant="warning" onClick={() => handleHistorico(responsavel.id)}>
                  Histórico
                </Button>
