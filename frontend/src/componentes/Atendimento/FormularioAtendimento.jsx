...
function FormularioAtendimento({
  ...
  // 🔹 Atualiza os campos quando formValues mudar (edição)
  useEffect(() => {
    if (formValues) {
      const d = formValues.data.split('T')[0]?.split('/');
      const formattedDate = `${d[2]}-${d[1]}-${d[0]}`;
      setAtendimento({
        status: formValues.status || '',
        tipo: formValues.tipo || '',
        data: formValues.data ? formattedDate : '',
        nome: formValues.nome || '',
        motivo: formValues.motivo || '',
        resolucao: formValues.resolucao || '',
        id: formValues.id,
      });
    } else {
      setAtendimento({
  ...
  useEffect(() => {
    ...
    // Se tipo for 1 ou 2, busca a lista de nomes
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5001/api/atendimentos/adicionar`,
        {
          method: 'POST',
          headers: {
  ...
  const renderCampoNome = () => {
    ...
    if (atendimento.tipo === '1' || atendimento.tipo === '2') {
      return (
        ...
        >
          <option value=''>
            {listaNomes.length ? 'Selecione um nome...' : 'Carregando nomes...'}
          </option>
