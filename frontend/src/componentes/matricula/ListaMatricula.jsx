...
const ListaMatriculasModal = ({ show, onHide, onSelectMatricula }) => {
  ...
  const handleDetalhes = async (id) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(
        `http://localhost:5001/api/matricula/matricula/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
            ...
            <tbody>
              {matriculas.map((matricula) => (
                <tr key={matricula.id}>
                  ...
                  <td>{matricula.turma_nome || 'Sem turma'}</td>
                  <td>{matricula.responsavel_nome}</td>
                  <td>
                    <Button
                      variant='primary'
                      onClick={() => handleDetalhes(matricula.id)}
                    >
                      Detalhes
                    </Button>
                    <Button
                      variant='success'
                      onClick={() => onSelectMatricula(matricula)}
